...
from Modeling.model.function import get_cores, update_cores  # 导入Tucker分解函数
...
if __name__ == '__main__':
    ...
    # Tucker分解参数初始化
    Js = data_set['train_input'].shape[1:]  
    Rs = (16, 16, 16) 
    Ms = initilizer(len(data_set['train_input']), Js, Rs, data_set['train_input'])

    # 核心张量
    core_tensors = get_cores(data_set['train_input'], Ms)
    data_set['train_input'] = torch.from_numpy(core_tensors).float()

    model = TreeCNs(
        core_dims=core_tensors[0].shape,  # 核心张量维度参数
        num_nodes=args.N,
